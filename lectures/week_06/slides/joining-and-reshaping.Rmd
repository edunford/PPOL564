...
- `index`: name of the variable that will be spread out into columns. 
- `values`: values that will populate the cells of each column
- `columns`: variables to leave alone.
...
```{python}
dat2.pivot_table(values='ln_gdppc', index='year', columns='country')
```
